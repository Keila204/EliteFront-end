...
export default function CarsControl() {
    ...
    async function apagarVeiculo(id) {
      ...
      confirmAlert({
        title: 'Veículo',
        message: 'Tem certeza que quer apagar o veículo?',
        buttons: [
          {
            label: 'Sim',
            onClick: async () => {
    ...
    async function salvar() {
      try {
        ...
        if (id == 0) {
          ...
          let r = await axios.put('http://localhost:5000/veiculo/' + id + veiculo);
                  ...
                  {listaVeiculos.map(item =>
                    <tr>
                      ...
                      <td className='btns' style={{ display: 'flex', height: 20 }}>
                        <i class="fa-regular fa-pen-to-square" onClick={() => alterarVeiculo(item)}></i>
                        <i class="fa-solid fa-delete-left" onClick={() => apagarVeiculo(item.id)}></i>
                      </td>
                    </tr>  
                  )}
